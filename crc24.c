...
/* CRC24 table - FCS - For GPRS: not compatible with BLE but I use it anyway */
...
#define INIT_CRC24	0x555555UL

static uint32_t crc24_calc_(uint32_t fcs, uint8_t *cp, unsigned int len) {
	while (len--)
...
uint32_t crc24_calc(uint8_t *cp, unsigned int len) {
	return crc24_calc_(INIT_CRC24, cp, len);
}
